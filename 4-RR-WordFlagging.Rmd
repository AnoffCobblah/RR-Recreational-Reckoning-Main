...
This script is a bit faster, because it doesn't attempt to keep track of the lemma %. Instead, it judges based on the sentences in which the match appears.
...
```{r Scatterplot Multi Text, eval=FALSE}
  for(i in 1:length(flagterm)) {
    ...
    tempdatadf <- subset(WordFlagdf,WordFlagdf$Searched_Term == flagterm[i])
...
```{r Scatterplot Multi Text subset, eval=FALSE}
  for(i in 1:length(flagterm)) {
    temptitle <- paste0("Appearances of '",flagterm[i],"' within Texts")
    tempdatadf <- WordFlagdf[grep(flagterm[i],WordFlagdf$Searched_Term),]
